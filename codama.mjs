...
// Source IDL is the spec-0.1.0 file `anchor build` writes to `target/idl` and
// `pnpm generate:idls` copies into `program/` (gitignored `target/` is not
// available to the client-generation CI job, and committing the IDL is what
// lets CI check both it and the clients for drift).
const idlPath = path.join(__dirname, 'program', 'idl.json');
...
// Render Umi.
// The umi renderer's built-in prettier defaults match `.prettierrc.json` except
// for the organize-imports plugin, so supply the plugin list (its own parser
// plugins included, since `plugins` replaces rather than extends). Without it
// the emitted import order fails `pnpm clients:umi:format`.
...
console.log('✔ Generated js, umi and rust clients from program/idl.json');
