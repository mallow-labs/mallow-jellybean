use crate::{
    ...
    get_config_count, JellybeanError, JellybeanMachine,
};
...
pub fn remove_multiple_items(
    ...
    require!(!indices.is_empty(), JellybeanError::InvalidInputLength);
    ...
    for i in 1..sorted_indices.len() {
        require!(
            ...
            JellybeanError::DuplicateIndex
        );
    }
    require!(
        sorted_indices.last().unwrap() < &(count),
        JellybeanError::IndexGreaterThanLength
    );
    ...
    for &index in indices.iter() {
        ...
        require!(
            ...
            JellybeanError::NumericalOverflowError
        );

        let config_line_position = BASE_JELLYBEAN_MACHINE_SIZE + 4 + index_usize * config_line_size;

        // Verify seller and authority
        let seller =
            Pubkey::try_from(&data[config_line_position + 32..config_line_position + 64]).unwrap();
        require!(
            authority == jellybean_machine.authority || seller == authority,
            JellybeanError::InvalidAuthority
        );
        require!(expected_seller == seller, JellybeanError::InvalidSeller);

        // Verify mint
        let item_mint =
            Pubkey::try_from(&data[config_line_position..config_line_position + 32]).unwrap();
        require!(mint == item_mint, JellybeanError::InvalidMint);

        // Verify amount for version 2+
        if jellybean_machine.version >= 2 {
            ...
            require!(amount == item_amount, JellybeanError::InvalidAmount);
        }
        ...
        let last_config_line_position =
            BASE_JELLYBEAN_MACHINE_SIZE + 4 + last_valid_usize * config_line_size;
        ...
        // Update mint indices
        let indices_start = jellybean_machine.get_mint_indices_position()?;
    ...
    // Update final count
    data[BASE_JELLYBEAN_MACHINE_SIZE..BASE_JELLYBEAN_MACHINE_SIZE + 4]
...
pub fn remove_multiple_items_span(
    ...
    require!(start_index <= end_index, JellybeanError::InvalidInputLength);
    ...
    require!(end_index < count, JellybeanError::IndexGreaterThanLength);
    ...
    if end_index < count - 1 {
        ...
        if items_after_removal > 0 {
            let source_start =
                BASE_JELLYBEAN_MACHINE_SIZE + 4 + ((end_index + 1) as usize) * config_line_size;
            let dest_start =
                BASE_JELLYBEAN_MACHINE_SIZE + 4 + (start_index as usize) * config_line_size;
    ...
    let zero_start =
        BASE_JELLYBEAN_MACHINE_SIZE + 4 + ((count - items_to_remove) as usize) * config_line_size;
